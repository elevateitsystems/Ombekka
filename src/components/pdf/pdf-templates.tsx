    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.table}>
        ...
        <View style={[styles.tableRow, styles.tableHeader]}>
          <View style={[styles.tableCol, { width: "15%" }]}><Text style={styles.tableColHeader}>Date</Text></View>
          <View style={[styles.tableCol, { width: "25%" }]}><Text style={styles.tableColHeader}>Event</Text></View>
          <View style={[styles.tableCol, { width: "30%" }]}><Text style={styles.tableColHeader}>Players</Text></View>
          <View style={[styles.tableCol, { width: "10%", textAlign: "center" }]}><Text style={styles.tableColHeader}>Result</Text></View>
          <View style={[styles.tableCol, { width: "20%" }]}><Text style={styles.tableColHeader}>Opening</Text></View>
        </View>
        ...
        {games.map((game, index) => (
          <View key={index} style={styles.tableRow}>
            <View style={[styles.tableCol, { width: "15%" }]}>
              <Text style={{ fontSize: 8 }}>{game.datePlayed ? new Date(game.datePlayed).toLocaleDateString() : "N/A"}</Text>
            </View>
            <View style={[styles.tableCol, { width: "25%" }]}>
              <Text style={{ fontSize: 8, fontWeight: "bold" }}>{game.tournament?.event || "—"}</Text>
              <Text style={{ fontSize: 7, color: "#64748b" }}>{game.tournament?.place || ""}</Text>
            </View>
            <View style={[styles.tableCol, { width: "30%" }]}>
              <Text style={{ fontSize: 8 }}>W: {game.white.name} ({game.whiteElo})</Text>
              <Text style={{ fontSize: 8 }}>B: {game.black.name} ({game.blackElo})</Text>
            </View>
            <View style={[styles.tableCol, { width: "10%", textAlign: "center" }]}>
              <Text style={{ fontSize: 8, fontWeight: "bold" }}>{game.result}</Text>
            </View>
            <View style={[styles.tableCol, { width: "20%" }]}>
              <Text style={{ fontSize: 8, color: "#4f46e5", fontWeight: "bold" }}>{game.ecoCode}</Text>
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={{ marginBottom: 20 }}>
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoValue}>{game?.tournament?.event || "N/A"}</Text>
        </View>
        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Date Played</Text>
          <Text style={styles.infoValue}>{game?.datePlayed ? new Date(game.datePlayed).toLocaleDateString(undefined, { dateStyle: 'full' }) : "N/A"}</Text>
        </View>
        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Location</Text>
          <Text style={styles.infoValue}>{game?.tournament?.place} ({game?.tournament?.federation || "N/A"})</Text>
        </View>
        <View style={styles.infoRow}>
      ...
      <View style={styles.openingBox}>
        ...
        {game?.eco && (
          <Text style={{ fontSize: 8, color: "#6366f1", marginTop: 5, fontFamily: "Helvetica-Oblique" }}>
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={{ marginTop: 20 }}>
        ...
        <Text style={{ fontSize: 10, lineHeight: 1.5, marginBottom: 10 }}>
          1. Acceptance of Terms: By downloading this PDF document, you agree to comply with and be bound by the following terms and conditions of use.
        </Text>
        
        <Text style={{ fontSize: 10, lineHeight: 1.5, marginBottom: 10 }}>
          2. Data Accuracy: The information provided in this report is generated based on available chess databases. While we strive for accuracy, Pawnder Info does not guarantee the completeness or absolute correctness of the data.
        </Text>
        
        <Text style={{ fontSize: 10, lineHeight: 1.5, marginBottom: 10 }}>
          3. Usage Rights: This document is for personal or research use only. Commercial redistribution or modification of this report without explicit written consent from Pawnder Info is strictly prohibited.
        </Text>
        
        <Text style={{ fontSize: 10, lineHeight: 1.5, marginBottom: 10 }}>
          4. Watermarking: Removing or obscuring the Pawnder Info watermark and logo from this document is a violation of these terms.
        </Text>
        
        <Text style={{ fontSize: 10, lineHeight: 1.5, marginBottom: 10 }}>
          5. Liability: Pawnder Info shall not be held liable for any decisions made based on the analysis provided in this report.
        </Text>
        
        <Text style={{ fontSize: 10, lineHeight: 1.5, marginTop: 20 }}>
          By clicking "I Agree" in the application interface, you acknowledge that you have read, understood, and agree to these terms.
        </Text>
      </View>
